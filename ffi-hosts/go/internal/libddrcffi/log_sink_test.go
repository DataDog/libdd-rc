...
// a repeat call is rejected without disturbing the fd it was given. This has
// to be one test rather than several: rc_enable_log_sink installs a
// process-global subscriber, so only the first call across the whole test
// binary can ever succeed.
