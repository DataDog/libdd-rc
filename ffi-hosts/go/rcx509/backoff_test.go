...
func TestBackoffNextGrowsAndCapsAtMax(t *testing.T) {
	b := newBackoff(1*time.Second, 8*time.Second, 2.0)
	...
	assertInRange(t, b.Next(), 4*time.Second, 8*time.Second)        // base 8s, capped
	assertInRange(t, b.Next(), 4*time.Second, 8*time.Second)        // base 16s, capped at 8s
}
...
func TestBackoffResetReturnsToInitialRange(t *testing.T) {
	b := newBackoff(1*time.Second, 30*time.Second, 2.0)
